...
// Player A to choose an asset that's available in Pyth and inputs the length of the bet
// Other players can see all existing bets (Fetch Bets)
// Player B can MATCH any of the bets and inputs his own prediction
// If the timing is over whoever is closest to the prices can claim the WHOLE amount
...
use crate::{state::*, constants::*, lib_utils::*, error::*};

declare_id!("5vCRNaJWAyCenKzfS9HiAFcHuUoRtFMAA35vgpgoVeyv");
...
mod stock_prediction {
    ...
    pub fn create_bet(
        ...
        // Transfer amount to the Bet PDA
    ...
    pub fn claim_bet(ctx: Context<ClaimBet>) -> Result<()> {
        // Check winner and send the prize to the winner
        // OR return them back the money if it's a draw
        let bet = &mut ctx.accounts.bet;
...
pub struct ClaimBet<'info> {
    ...
    // Unwrapping because player b is an option
    #[account(mut, address = bet.prediction_b.as_ref().unwrap().player)]
    pub player_b: AccountInfo<'info>,

    // The person signing this claim (could be player A or B)
    #[account(mut)]
